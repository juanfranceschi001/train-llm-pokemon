...
Architecture ported with minimal changes from orbek/train-llm (Apache-2.0);
see NOTICE. It is dataset-agnostic -- nothing here is Shakespeare- or
Pokemon-specific, only `vocab_size` changes based on what you train on.
"""
import math
...
# The "full" model used for the committed training run, and a tiny debug preset.
...
def get_device():
    ...
    Works seamlessly on Windows/Linux (CUDA or CPU), macOS (MPS or CPU), and
    Google Colab (CUDA when a GPU runtime is selected, else CPU)."""
